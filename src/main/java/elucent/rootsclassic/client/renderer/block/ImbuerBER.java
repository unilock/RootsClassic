...
public class ImbuerBER implements BlockEntityRenderer<ImbuerBlockEntity> {
    ...
    public void render(ImbuerBlockEntity imbuerTile, float partialTicks, PoseStack matrixStackIn, MultiBufferSource bufferIn, int combinedLightIn, int combinedOverlayIn) {
        ...
        if (!stickStack.isEmpty()) {
            ...
            Minecraft.getInstance().getItemRenderer().renderStatic(stickStack, ItemTransforms.TransformType.GROUND, combinedLightIn, combinedOverlayIn, matrixStackIn, bufferIn, 0);
        ...
        if (!dustStack.isEmpty()) {
            ...
            Minecraft.getInstance().getItemRenderer().renderStatic(dustStack, ItemTransforms.TransformType.GROUND, combinedLightIn, combinedOverlayIn, matrixStackIn, bufferIn, 0);
            matrixStackIn.popPose();
        }
    }
}
